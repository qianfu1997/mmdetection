...
model = dict(
    ...
    bbox_roi_extractor=dict(
        type='PANMultiRoIExtractor',          # change roi extractor
        roi_layer=dict(type='RoIAlign', out_size=7, sample_num=2),
        out_channels=256,
        featmap_strides=[4, 8, 16, 32]),
    bbox_head=dict(
        type='PANSharedFCBBoxHead',             # change bbox head
        num_fcs=2,
        in_channels=256,
        ...
        len_strides=4),                          # same with featmap_strides.
    mask_roi_extractor=dict(
        type='PANMultiRoIExtractor',       #   change to multi
        roi_layer=dict(type='RoIAlign', out_size=14, sample_num=2),  # ori 14
        out_channels=256,
        featmap_strides=[4, 8, 16, 32]),
    mask_head=dict(
        type='FCNMaskHeadFusionPAN',          # change to
        num_convs=4,
        in_channels=256,
