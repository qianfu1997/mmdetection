...
model = dict(
    ...
    mask_roi_extractor=dict(
        ...
        featmap_strides=[4, 8, 16, 32]),
    mask_head=dict(
        type='FCNDiceMaskHead',         # use the custom FCNDiceMaskHead
        num_convs=4,                    # use dice loss only, set num_classes to 2
        in_channels=256,                # use combined loss, set num_classes to 2
