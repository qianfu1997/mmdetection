...
class FCNResMaskHead(nn.Module):
    def __init__(self,
                 num_res_convs=4,       # convs in each res block
                 num_res_block=2,       # total num of res block
                 roi_feat_size=14,
                 in_channels=256,
                 conv_kernel_size=3,
                 ...
                 normalize=None):
        super(FCNResMaskHead, self).__init__()
        self.num_res_convs = num_res_convs
        ...
        for i in range(self.num_res_block):
            ...
            self.res_layers.append(
                ConvModule(
                    ...
                    bias=self.with_bias,
                    activation=None))
        for i in range(self.num_res_block * self.num_res_convs):
            in_channels = (self.in_channels if i == 0 else self.conv_out_channels)
            padding = (self.conv_kernel_size - 1) // 2
            activation = None if i % self.num_res_convs == 0 else 'relu'
            self.convs.append(
                ConvModule(
                    in_channels,
                    self.conv_out_channels,
    ...
    def forward(self, x):
        for i in range(self.num_res_block):
            ...
            for j in range(self.num_res_convs):
                x = self.convs[i * self.num_res_convs + j](x)
            x = x + res
            x = F.relu(x)
