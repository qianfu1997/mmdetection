...
input_dir = '../input_dir/'
gt_dir = '../gt_dir/IC19ArT_val.odgt'
fid_path = '../res_tt.txt'

allInputs = listdir(input_dir)

"""read gt from odgt"""
with open('/home/xieenze/DataSet/IC19ArT/IC19ArT_val.odgt','r') as f:
	data = f.readlines()
...
def gt_reading_mod(gt_dir, gt_id):
	gt_id = gt_id[8:].replace(".txt","")
	gt_id = 'gt_{}.jpg'.format(gt_id)
	gt_data = d[gt_id]['gtmasks']
	results = []
	for i in gt_data:
		points = i['mask']
		x = points[0][::2]
		y = points[0][1::2]
		result = []
		result.append(np.array(['x:'],dtype='<U2')); result.append(np.array(x,dtype='int16'));
		result.append(np.array(['y:'],dtype='<U2')); result.append(np.array(y,dtype='int16')); 
		result.append(np.array(['clear'],dtype='<U5')); result.append(np.array(['c'],dtype='<U1'))
